...
export function calculateTotal(items) {
  ...
  const reduceCallback = function(acc, curr) {
    return acc += (curr.price * curr.qty);
  }

  return items.reduce(reduceCallback, 0);
}
...
export function sortByPrice(items, ascending) {
  ...
  if (ascending) {
    ...
    sortCallback = (a, b) => a.price - b.price;
  } else {

    sortCallback = (a, b) => b.price - a.price;
  }

  return itemsSorted.sort(sortCallback);
}
...
export function formatBill(items) {
  ...
  const newItems = items.map((item) => `${item.name} x ${item.qty} = Rs.${item.price * item.qty}`);
